...
m, p, q, n = 4, 3, 3, 4; @assert m >= p && n >= q # (3,3,4,4,  2)
#r = 2;  @assert 0 <= r <= min(p,q) # maximum rank permitted of X
timeout = 60.0; # in seconds; MUST include a decimal point

@var A[1:m,1:n];
@var X[1:p, 1:q];
@var B[1:m, 1:p], C[1:q, 1:n];
critical_equations = differentiate(f(A - B*X*C), vec(X));

lower_triangular_indices = [(i,j) for i ∈ 1:p, j ∈ 1:q if i > j];
symmetric_constraints = [X[i,j]-conj(X[j,i]) for (i,j) in lower_triangular_indices];

parametric_system = System(vcat(critical_equations,symmetric_constraints);
                           parameters = vcat(vec.([A, B, C])...));
...
if use_default_start_pair
    monodromy_result =
        monodromy_solve(
            parametric_system;
            #target_solutions_count=binomial(min(p,q),r),
            timeout=timeout);
else
    monodromy_result = 
        monodromy_solve(
...
xs = [reshape(target_solutions[i][1:p*q], p, q) for i = 1:length(target_solutions)];
foreach(display, xs)
foreach(display ∘ issymmetric, xs)
...
V_B, sf, U_C, sz = solution_factors(A_0, B_0, C_0, rrrrr);
